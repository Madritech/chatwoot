...
import CampaignWhatsAppOptions from '../../../../NewConversation/components/CampaignWhatsAppOptions.vue';
...
const selectedInbox = computed(() =>
  formState.inboxes.value
    .find(inbox => inbox.id === state.inboxId)
);

const messageTemplates = computed(() => selectedInbox.value?.message_templates?.filter(messageTemplates => messageTemplates.status === 'APPROVED') || []
);
...
const inboxOptions = computed(() => {
  return mapToOptions(formState.inboxes.value, 'id', 'name')
}
...
<template>
  <form class="flex flex-col gap-4" @submit.prevent="handleSubmit">
    ...
    <div class="flex items-center justify-between">
      <CampaignWhatsAppOptions
        :message-templates="messageTemplates"
        @selected-template-data="({ name, placeholders, lang_code }) => {
          const placeholderPart = placeholders.length ? `:${placeholders.join('|')}` : '';
          state.message = `${name}::${lang_code}${placeholderPart}`;
        }"
      />
      <div v-if="state.message" class="ml-4 text-sm text-n-slate-11">
        <strong>{{ state.message.split('::')[0] }}</strong>
      </div>
    </div>
